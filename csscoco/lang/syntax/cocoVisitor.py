# Generated from java-escape by ANTLR 4.5
from antlr4 import *

# This class defines a complete generic visitor for a parse tree produced by cocoParser.

class cocoVisitor(ParseTreeVisitor):

    # Visit a parse tree produced by cocoParser#stylesheet.
    def visitStylesheet(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#context.
    def visitContext(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#convention.
    def visitConvention(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#pattern.
    def visitPattern(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#fork_pattern.
    def visitFork_pattern(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#node_declaration.
    def visitNode_declaration(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#semantic_node.
    def visitSemantic_node(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#node_type.
    def visitNode_type(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#whitespace_variation.
    def visitWhitespace_variation(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#whitespace_node.
    def visitWhitespace_node(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#logic_expr.
    def visitLogic_expr(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#type_expr.
    def visitType_expr(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#arithmetic_expr.
    def visitArithmetic_expr(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#element.
    def visitElement(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#call_expr.
    def visitCall_expr(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#repeater.
    def visitRepeater(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#ignore_clause.
    def visitIgnore_clause(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#list_.
    def visitList_(self, ctx):
        return self.visitChildren(ctx)


    # Visit a parse tree produced by cocoParser#list_element.
    def visitList_element(self, ctx):
        return self.visitChildren(ctx)


